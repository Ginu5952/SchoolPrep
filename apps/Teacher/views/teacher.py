...
def teacher_info(request):
    if request.user.is_authenticated:
        try:
            teacher = Teacher.objects.get(user=request.user)  # Assuming the Teacher model has a OneToOneField with User
            serializer = TeacherSerializer(teacher)  # Serializing the teacher object
            return Response(serializer.data, status=status.HTTP_200_OK)
...
def class_detail(request):
    if request.user.is_authenticated:
        try:
            # Assuming the Class model has a field `teacher` that links to the logged-in user
